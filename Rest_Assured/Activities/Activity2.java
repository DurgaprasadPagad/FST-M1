...
public class Activity2 {
	...
	public void getUserInfo() {
		...
		response.then().body("username", equalTo("durgapagad"));
		response.then().body("firstName", equalTo("Durgarpasad"));
		response.then().body("lastName", equalTo("Pagad"));
		response.then().body("email", equalTo("durgapagad@mail.com"));
		response.then().body("password", equalTo("password123"));
		response.then().body("phone", equalTo("9987401138"));
	}
	...
	public void deleteUser() throws IOException {
		...
		// Assertion
		response.then().body("code", equalTo(200));
		response.then().body("message", equalTo("durgapagad"));
	}
}
